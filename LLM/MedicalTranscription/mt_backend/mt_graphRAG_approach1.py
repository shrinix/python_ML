...
async def build_graph(llm,embedder, text, entities, relations, potential_schema, prompt_template) -> PipelineResult:
    ...
    res = await define_and_run_pipeline(driver, llm, embedder, text, entities, relations, potential_schema, prompt_template)
...
def get_llm_and_embedder(useOllama):
    ...
    if useOllama:
        ...
        # embedder=neo4j_graphrag.embeddings.OpenAIEmbeddings(model=""),

    return llm, embedder

def initialize(useOllama):

    home_dir = os.path.expanduser("~")
    # data_dir = "../data/"
    llm, embedder = get_llm_and_embedder(useOllama)
    ...
    print("Initialization of LLM and Embedder completed")
    return prompt_template, llm, embedder

# Endpoint to load data
@app.route('/load_data', methods=['POST'])
def load_data_and_build_graph(llm, embedder, prompt_template):
    # data_dir = request.json.get('data_dir', "../data/")
    try:
        # Connect to the Neo4j database
        driver = neo4j.GraphDatabase.driver(NEO4J_URI, auth=NEO4J_AUTH,database=NEO4J_DATABASE)

        text = ("Medical transcription for {candidate} is given below."
        ...
        text = text.format(candidate=candidate)
        ...
        res = asyncio.run(build_graph(llm, embedder, text, entities, relations, potential_schema, prompt_template))
        print(res)
...
def chat_endpoint():
    ...
    try:
        ...
        rag = GraphRAG(retriever=retriever, llm=llm)
        result = rag.search(query_text=query, retriever_config={"top_k": 5})
    except Exception as e:
        print(f"An error occurred: {e}")
        logging.error("An error occurred: %s", e)
    finally:
...
if __name__ == "__main__":
    ...
    candidate = "Patient_P001" #"Patient_ABC"
    useOllama = False
    # TODO: Need to fix the app to run with Ollama. SimpleKGPipeline does not work with Ollama.
    # Initialize the application
    prompt_template, llm, embedder = initialize(useOllama)

    rebuild = False
    if rebuild:
        load_data_and_build_graph(llm, embedder, prompt_template)

    # uncomment this line to run the app without front-end
    # create_retriever_and_do_rag(candidate)

    #comment this line to run the app without front-end
