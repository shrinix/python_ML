...
#TODO: Fix neo4j driver memory leak with this function
def create_vector_index(index_name, node_label, text_node_properties, embedding_node_property, embedding, url,auth):
...
if __name__ == "__main__":
    ...
    try:
        ...
        with neo4j.GraphDatabase.driver(NEO4J_URI, auth=NEO4J_AUTH, database=NEO4J_DATABASE) as driver:
            ...
            # '''
            vector_index = create_vector_index(VECTOR_INDEX_NAME, LABEL, [property_name], vector_property_name, embedder ,NEO4J_URI,NEO4J_AUTH)  
        ...
        # with neo4j.GraphDatabase.driver(NEO4J_URI, auth=NEO4J_AUTH, database=NEO4J_DATABASE) as driver:
            ...
            embedder = OpenAIEmbeddings(model="text-embedding-3-small")
            #The retriever needs to be instantiated within the  driver context to avoid a driver-related memory leak
