...
def print_prediction_summary(predictions, uniques, X_test, Y_test, encoder):
	...
	#print(numpy.column_stack((Y_test_uniques, pred_values, Y_test_uniques == pred_values)))
	...
	print(numpy.column_stack((X_test[Y_test_uniques != pred_values], Y_test_uniques[Y_test_uniques != pred_values], pred_values[Y_test_uniques != pred_values])))
...
def save_model(model):
	#generate a model name based on the current time in the format iris_classifier_yyyymmdd.h5
	#save the model to the models directory
	model_name = "iris_classifier"+datetime.datetime.now().strftime("%Y%m%d_%H%M%S")+".h5"
...
def load_model():
	...
	model = load_model("models/iris_classifier.h5")
	return model
